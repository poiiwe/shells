...
function change_client_interval() {
    ...
    read -rp "请输入新的上报间隔(秒，默认1): " new_interval
    ...
    if [[ ! "$new_interval" =~ ^[0-9]+$ ]] || [ "$new_interval" -lt 1 ]; then
        echo "[错误] interval 必须为大于0的数字"
        return 1
    fi

    CLIENT_INTERVAL="$new_interval"
    ...
    if systemctl is-active --quiet stat_client; then
        echo "[成功] 上报间隔已修改为 ${CLIENT_INTERVAL}s"
    else
        echo "[错误] 服务启动失败"
        journalctl -u stat_client -n 30 --no-pager
        return 1
    fi
}
...
# v1.7.10 service配置验证
# 检查生成后的 systemd 是否包含 interval
function verify_client_service() {

    local service_content

    service_content=$(systemctl cat stat_client 2>/dev/null)

    echo "$service_content" | grep -q -- "--interval"

    if [ $? -ne 0 ]; then
        echo "[警告] 未检测到 --interval 参数"
    fi
}
