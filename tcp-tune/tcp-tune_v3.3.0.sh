...
#   · 吞吐优先控制：≤0.3% 视为可接受，连续 2 次升 8%；>2% 降 8%，>5% 才降 15%
#   · 低流量每 2 分钟回升 3%，灰区稳定时每 2 分钟探测 2%；区间 [上限的 40%, 上限]
#   · 使用累计样本窗，低流量不再每轮丢弃样本，避免误判和降速后没有回调
...
#   · --curve 支持两位小数（0.75），默认爬升积极度由 0.7 优化为 0.8
# RTT 测量
#   · 新增 --rtt-host IP/域名，并在向导中支持 ping 5 次自动测量平均 RTT 后向上取整到 10ms
# fq_pie
...
measure_rtt() {
  local target=$1 output measured received ping_cmd=ping
  command -v ping >/dev/null 2>&1 || die "自动测量 RTT 需要 ping（通常由 iputils-ping 提供）"
  [[ -n "$target" && "$target" != -* && "$target" =~ ^[a-zA-Z0-9._:%-]+$ ]] || \
    die "RTT 测试目标不是有效的 IP 或域名：${target:-空}"

  # 旧版系统可能需要独立的 ping6；现代 iputils 的 ping 可直接识别 IPv6。
  if [[ "$target" == *:* ]] && command -v ping6 >/dev/null 2>&1; then ping_cmd=ping6; fi
  info "正在测试 $target 的 RTT（5 次，超时 2 秒/次）..."
  output=$(LC_ALL=C "$ping_cmd" -n -c 5 -W 2 "$target" 2>&1 || true)
  measured=$(printf '%s\n' "$output" | awk '
    {
      for (i=1; i<=NF; i++) {
        if ($i ~ /^time[=<][0-9.]+/) {
    ...
    END {
      if (n > 0) {
        avg=sum/n
        # 以 10ms 为单位向上取整：171.2 -> 180，180.0 -> 180，180.1 -> 190。
        rounded=int(avg/10); if (avg > rounded*10) rounded++
        rounded*=10
        if (rounded < 10) rounded=10
        printf "%d:%d:%.3f", rounded, n, avg
      }
    }')
  [[ -n "$measured" ]] || { printf '%s\n' "$output" >&2; die "无法从 $target 获得 RTT：请检查域名、网络或 ICMP 是否被禁止"; }
  IFS=: read -r RTT_MS received RTT_AVG_MS <<< "$measured"
  ok "RTT 测量完成：成功 $received/5，平均 ${RTT_AVG_MS} ms，向上取整到 10ms 为 ${RTT_MS} ms"
}
...
wizard() {
  ...
  local rtt_mode
  choose_option rtt_mode "RTT 获取方式" 1 \
    'test|输入 IP 或域名自动测试（推荐）' \
    'manual|手动输入 RTT'
  if [[ "$rtt_mode" == test ]]; then
    RTT_HOST=$(prompt_default "用于测试 RTT 的 IP 或域名" "120.222.50.77")
    measure_rtt "$RTT_HOST"
  else
    RTT_MS=$(prompt_default "典型往返 RTT（ms）" "150")
  fi
  ...
  CURVE=$(prompt_default "爬升积极度 0.1-1.0" "0.8")
...
preview_config() {
  ...
  say "  • 算法：$CC + $QDISC（$PROFILE，积极度 $CURVE）"
  say "  • 整形：$shape_text"
  ...
  if (( ADAPT )); then
    say "  • 调速：≤0.3% 重传升速；2%~5% 降 8%；>5% 降 15%"
  fi
...
# cake 的 bandwidth。低流量时也会定时回升、灰区会谨慎探测，避免降速后永久卡住。
# ADAPT=0 时顺手清理已装单元。
...
if command -v nstat >/dev/null 2>&1; then
  read -r out retr < <(nstat -az 2>/dev/null | awk '
    /^TcpOutSegs/{o=$2} /^TcpRetransSegs/{r=$2} END{if (o != "") print o+0, r+0}')
fi
if ! [[ "${out:-}" =~ ^[0-9]+$ && "${retr:-}" =~ ^[0-9]+$ ]]; then
  read -r out retr < <(awk '
    $1=="Tcp:" && !seen++ {for(i=2;i<=NF;i++) col[$i]=i; next}
    $1=="Tcp:" {print $(col["OutSegs"]), $(col["RetransSegs"]); exit}' /proc/net/snmp 2>/dev/null)
fi
[[ "${out:-}" =~ ^[0-9]+$ && "${retr:-}" =~ ^[0-9]+$ ]] || { echo "无法读取 TCP 计数器"; exit 0; }
now=$(date +%s)
...
: "${LAST_DROP_TS:=0}" "${LAST_CHANGE_TS:=0}" "${OK_COUNT:=0}"
...
if (( LAST_OUT == 0 )) || (( out < LAST_OUT )) || (( retr < LAST_RETR )); then
  (( LAST_DROP_TS > 0 )) || LAST_DROP_TS=$now
  (( LAST_CHANGE_TS > 0 )) || LAST_CHANGE_TS=$now
  save_state
...
if (( d_out >= 2000 )); then
  ...
  if (( ratio_ppm > 50000 )); then
    # >5% 属于严重拥塞，快速退让；避免普通的少量重传导致大幅降速。
    CUR_KBIT=$(( CUR_KBIT * 85 / 100 ))
    OK_COUNT=0
    LAST_DROP_TS=$now
    reason="严重重传"
  elif (( ratio_ppm > 20000 )); then
    # 2%~5% 温和退让 8%，相比旧版 >1% 直接降 15% 更偏向吞吐。
    CUR_KBIT=$(( CUR_KBIT * 92 / 100 ))
    OK_COUNT=0
    LAST_DROP_TS=$now
    reason="重传偏高"
  elif (( ratio_ppm <= 3000 )); then
    # ≤0.3% 是高带宽长 RTT 链路可接受的小量重传；连续两窗即积极回升。
    OK_COUNT=$(( OK_COUNT + 1 ))
    if (( OK_COUNT >= 2 && CUR_KBIT < CEIL_KBIT )); then
      step=$(( CUR_KBIT * 8 / 100 )); min_step=$(( CEIL_KBIT * 2 / 100 ))
    ...
    OK_COUNT=0
    # 0.3%~2% 不降速；稳定 5 分钟后每 2 分钟向上探测上限的 2%。
    if (( CUR_KBIT < CEIL_KBIT && now - LAST_DROP_TS >= 300 && now - LAST_CHANGE_TS >= 120 )); then
      CUR_KBIT=$(( CUR_KBIT + CEIL_KBIT * 2 / 100 ))
      reason="可接受重传区间探测"
    fi
  fi
  ...
  if tc qdisc replace dev "$IFACE" root "${new_args[@]}"; then
    ...
    if (( d_out > 0 )); then
      awk -v why="$reason" -v o="$old" -v n="$CUR_KBIT" -v r="$d_retr" -v t="$d_out" -v s="$d_sec" \
        'BEGIN{printf "整形 %d -> %d Kbit（%s；%d s 内重传 %d/%d = %.3f%%）\n", o, n, why, s, r, t, r*100/t}'
    else
...
ADAPT_EOF
...

  {
    printf 'IFACE=%q\n' "$IFACE"
    printf 'CEIL_KBIT=%s\n' "$CAKE_RATE_KBIT"
    printf 'FLOOR_KBIT=%s\n' "$(clamp "$(( CAKE_RATE_KBIT * 2 / 5 ))" 50000 100000000)"
    printf 'CUR_KBIT=%s\n' "$CAKE_RATE_KBIT"
    printf 'OK_COUNT=0\nLAST_OUT=0\nLAST_RETR=0\nLAST_TS=0\nLAST_DROP_TS=0\nLAST_CHANGE_TS=0\n'
    printf 'ARGS=(%s)\n' "$(printf '%q ' "${QDISC_ARGS[@]}")"
  } > "$ADAPT_STATE_FILE"
...
EOF
    ...
    if systemctl enable --now "$ADAPT_TIMER_NAME" >/dev/null 2>&1; then
      ok "已启用自适应整形：$ADAPT_TIMER_NAME（每 30s 调，上限 $((CAKE_RATE_KBIT/1000)) Mbps，下限 $((CAKE_RATE_KBIT*2/5000)) Mbps）"
    else
...
apply_config() {
  ...
  if ! { (( VERBOSE )) && sysctl -p "$CONFIG_FILE" || sysctl -p "$CONFIG_FILE" >/dev/null; }; then
  ...
  say
  ok "优化完成"
  say "  • 算法：$(sysctl -n net.ipv4.tcp_congestion_control 2>/dev/null || true) + $QDISC"
  if (( ADAPT )); then
    say "  • 整形：自适应，上限 $((CAKE_RATE_KBIT/1000)) Mbps，定时器已启用"
  elif [[ -n "$CAKE_RATE_KBIT" ]]; then
    say "  • 整形：固定 $((CAKE_RATE_KBIT/1000)) Mbps"
  else
    say "  • 整形：关闭"
  fi
  say "  • 配置：$CONFIG_FILE"
...
main() {
  ...
  case "$ACTION" in
    wizard) wizard ;;
    rtt)
      [[ -n "$RTT_HOST" ]] || die "rtt 操作需要 --rtt-host IP或域名"
      measure_rtt "$RTT_HOST"
      say "$RTT_MS"
      ;;
    preview|apply)
      if [[ -n "$RTT_HOST" ]]; then measure_rtt "$RTT_HOST"; fi
      [[ -n "$LOCAL_MBPS" && -n "$SERVER_MBPS" && -n "$RTT_MS" ]] || die "preview/apply 需要带宽与 RTT 参数（--rtt-ms 或 --rtt-host）；或使用 wizard"
